...
by_address = \
    joint[['price','display_address']].groupby('display_address').count().reset_index()
...
by_address = \
    joint[['price','display_address']].groupby('display_address').mean().reset_index()
...
#     "num_description_words","created_hour",'price_t1',\
#     'img_brg_mean','img_hgt_mean','img_sat_mean','img_wdt_mean',\
...
# coords = np.array([list(a) for a in zip(joint.latitude,joint.longitude)])
...
# out_df.to_csv("../output/xgb_starter_03_mods.csv", index=False)      